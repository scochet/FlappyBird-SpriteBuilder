...
- (void)update:(CCTime)delta
{
    ...
    for (CCNode *bush in _bushes) {
        // move the bush
        bush.position = ccp(bush.position.x - (character.physicsBody.velocity.x * delta * 0.5),
                            bush.position.y);
        // if the left corner is one complete width off the screen,
        // move it to the right
        if (bush.position.x <= (-1 * bush.contentSize.width)) {
            bush.position = ccp(bush.position.x + 2 * bush.contentSize.width,
                                bush.position.y);
        }
    }
    ...
    for (CCNode *cloud in _clouds) {
        // move the cloud
        cloud.position = ccp(cloud.position.x - (character.physicsBody.velocity.x * delta * 0.15),
                             cloud.position.y);
        // if the left corner is one complete width off the screen,
        // move it to the right
        if (cloud.position.x <= (-1 * cloud.contentSize.width)) {
            cloud.position = ccp(cloud.position.x + 2 * cloud.contentSize.width,
                                 cloud.position.y);
        }
    }
}
